...
# Usage: from repo root or evaluation_framework: ./evaluation_framework/scripts/judge.sh [input_json] [out_json] [model] [concurrency] [summary_json] [backend]
# Example: ./evaluation_framework/scripts/judge.sh output/unified_predictions.json output/judged.json gpt-4o-mini 4 output/judge_summary.json call_llm
set -e
...
echo "Input: $INPUT_FILE | Output: $OUT_FILE | Model: $MODEL | Backend: $BACKEND | Concurrency: $CONCURRENCY"
